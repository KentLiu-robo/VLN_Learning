...
## Questions
### Frontier Map如何利用在VLN探索过程中？
Frontier Map 是栅格地图，Frontier是已知区域和未知区域的边界，其组成的单元定义：
````text
当前格子是 free，并且它的邻域中至少有一个 unknown 格子
````
由这些单元格组成出完整的Frontier。

如何使用Frontier Map做目标导航？
#### 1：构建 occupancy grid map：
机器人每一帧获得 RGB-D 或深度图，然后结合位姿估计，把深度点投影到 2D 栅格地图。
...
```
最基本的做法是：
1. 从机器人位置向每个深度点做 ray casting；
2. 射线路径上的格子标记为 free；
3. 深度终点位置标记为 occupied；
4. 未被观测到的区域保持 unknown。
结果得到一张 2D occupancy map。
#### 2.获取frontier cells：
对每个 free cell 检查8邻域，找到满足条件的cell
#### 3.对 frontier cells 做聚类
把单个的frontier cells聚类成frontier clusters, 每个cluster可以视作一个探索候选区域并保存：
````text
cluster_id
frontier cells
中心点 centroid
...
````
#### 4.过滤不可用frontier
不是所有 frontier 都值得去。需要过滤掉：
```
面积太小的 frontier
靠障碍物太近的 frontier
A* / Dijkstra 不可达的 frontier
距离太近、没有信息增益的 frontier
已经访问多次但没有新信息的 frontier
```
#### 5.借鉴Apex-Nav给每个frontier 增加一个semantic score
仿照ApexNav 使用 BLIP-2 计算图文匹配分数，并把分数投影到已观测 free grids 上，用来给 frontier 打分，这样就可以构建semantic score map，并主动进行探索未知区域，前往最有可能完成目标的区域。