...
提出 Human Attention Aided Transformer（HAA-Transformer），不仅预测无人机下一步 waypoint，还同时预测人类 follower 在图像中关注的位置。论文实验显示，加入 human attention prediction 后，Transformer 和 LSTM 两类模型性能都有提升，尤其对长轨迹更有帮助。

```text
历史对话 → BERT Encoder → Language Embeddings
无人机视觉观察 → xView-pretrained DarkNet-53 + Attention → Visual Embeddings
无人机方向 → Direction Encoder → Direction Embeddings
